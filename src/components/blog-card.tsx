  ...
  return (
    <div className="group relative h-full">
      <Link href={`/blog/${post.slug}`} className="block h-full">
        {/* 核心：p-0 确保没有内边距，overflow-hidden 确保图片圆角 */}
      ...
      {isAdmin && (
        <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 z-20">
          <Link href={`/admin/write/${post.id}`}>
            <div className="p-2 bg-white/80 backdrop-blur-sm text-blue-600 rounded-full shadow-lg hover:bg-blue-600 hover:text-white border border-zinc-200 transition-all">
              <Edit size={14} />
            </div>
          </Link>
          <button 
            onClick={handleDelete}
            className="p-2 bg-white/80 backdrop-blur-sm text-red-600 rounded-full shadow-lg hover:bg-red-600 hover:text-white border border-zinc-200 transition-all"
          >
            <Trash2 size={14} />
          </button>
        </div>
